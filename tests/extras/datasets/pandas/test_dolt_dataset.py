...
@pytest.fixture
def filepath_dolt(tmp_path):
    return (tmp_path / "testdb").as_posix()


@pytest.fixture(scope="function")
def doltdb(filepath_dolt):
    try:
        db = Dolt.init(filepath_dolt)
        db.sql("create table  t1 (col1 bigint, col2 bigint, col3 bigint)")
        db.sql("insert into t1 values (1,4,5), (2,5,6)")
        db.sql("select dolt_add('t1')")
        db.sql("select dolt_commit('-m', 'initialize t1')")
        yield filepath_dolt
    finally:
        if os.path.exists(filepath_dolt):
            shutil.rmtree(filepath_dolt)


@pytest.fixture
def dolt_data_set(filepath_dolt):
    dolt.Dolt.init(filepath_dolt)
    return DoltDataSet(
        filepath=filepath_dolt,
        tablename="t1",
        index=["col1"],
        meta_config=dict(
            filepath=filepath_dolt,
...
@pytest.fixture
def dummy_dataframe():
    return pd.DataFrame({"col1": [1, 2], "col2": [4, 5], "col3": [5, 6]}).set_index("col1")


class TestDoltDataSet:
    def test_save_and_load(self, dolt_data_set, dummy_dataframe):
        """Test saving and reloading the data set."""
        dolt_data_set.save(dummy_dataframe)
        reloaded = dolt_data_set.load()
        assert_frame_equal(dummy_dataframe, reloaded)
        assert dolt_data_set._fs_open_args_load == {}
        assert dolt_data_set._fs_open_args_save == {"mode": "w", "newline": ""}

    def test_exists(self, dolt_data_set, dummy_dataframe):
        """Test `exists` method invocation for both existing and
        nonexistent data set."""
        assert not dolt_data_set.exists()
        dolt_data_set.save(dummy_dataframe)
    ...
    def test_load_missing_file(self, csv_data_set):
        ...
        pattern = r"Failed while loading data from data set DoltDataSet\(.*\)"
    ...
    def test_protocol_usage(self, filepath, instance_type, credentials):
        data_set = DoltDataSet(filepath=filepath, credentials=credentials)
    ...
    def test_catalog_release(self, mocker):
        ...
        filepath = "test.csv"
        data_set = DoltDataSet(filepath=filepath)
...
class TestDoltDataSetVersioned:
    def test_version_str_repr(self, load_version, save_version):
        """Test that version is in string representation of the class instance
        when applicable."""
        filepath = "test.csv"
        ds = DoltDataSet(filepath=filepath)
        ds_versioned = DoltDataSet(
        ...
        assert "DoltDataSet" in str(ds_versioned)
        assert "DoltDataSet" in str(ds)
    ...
    def test_multiple_loads(
        ...
        DoltDataSet(filepath=filepath_csv, version=Version(v_new, v_new)).save(
            dummy_dataframe
        )
        ...
        ds_new = DoltDataSet(filepath=filepath_csv, version=Version(None, None))
        assert (
    ...
    def test_multiple_saves(self, dummy_dataframe, filepath_csv):
        ...
        ds_versioned = DoltDataSet(filepath=filepath_csv, version=Version(None, None))
    ...
    def test_release_instance_cache(self, dummy_dataframe, filepath_csv):
        ...
        ds_a = DoltDataSet(filepath=filepath_csv, version=Version(None, None))
        assert ds_a._version_cache.currsize == 0
        ds_a.save(dummy_dataframe)  # create a version
        assert ds_a._version_cache.currsize == 2

        ds_b = DoltDataSet(filepath=filepath_csv, version=Version(None, None))
    ...
    def test_no_versions(self, versioned_csv_data_set):
        ...
        pattern = r"Did not find any versions for DoltDataSet\(.+\)"
        with pytest.raises(DataSetError, match=pattern):
            versioned_csv_data_set.load()
    ...
    def test_prevent_overwrite(self, versioned_csv_data_set, dummy_dataframe):
        ...
        versioned_csv_data_set.save(dummy_dataframe)
        pattern = (
            r"Save path \`.+\` for DoltDataSet\(.+\) must "
    ...
    def test_save_version_warning(
        ...
        pattern = (
            ...
            r"for DoltDataSet\(.+\)".format(save_version, load_version)
        )
